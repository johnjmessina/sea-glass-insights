...
function defaultMeta(): MetaMap {
  return Object.fromEntries(
    SYNTHETIC_SECTIONS.map(s => [s.key, { notes: "", locked: false, callout: "" }])
  );
}
...
export default function SyntheticSurveyDetail({ order: initialOrder, onBack }: Props) {
  ...
  const lockedCount = SYNTHETIC_SECTIONS.filter(s => meta[s.key]?.locked).length;
  const allLocked   = lockedCount === SYNTHETIC_SECTIONS.length;
  ...
  const questionLabels = getQuestionLabels("synthetic_survey_report");
  ...
  async function downloadDocx() {
    ...
    try {
      const analystPerspectives = Object.fromEntries(
        SYNTHETIC_SECTIONS.map(s => [s.key, meta[s.key]?.callout ?? ""])
      );
      const res = await fetch("/api/generate-ssr-pdf", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
  ...
  // ── Render one AI section (idx is 0-based) ────────────────────────────────────

  function renderAISection(idx: number) {
    const section        = SYNTHETIC_SECTIONS[idx];
    ...
    return (
      <div key={key}>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <span className="text-xs font-bold text-violet-600 bg-violet-50 px-2.5 py-1 rounded-full shrink-0">
            Section {idx + 1} of {SYNTHETIC_SECTIONS.length}
          </span>
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100">
          ...
          <button
            type="button"
            onClick={() => isLast ? setActiveSection(SYNTHETIC_SECTIONS.length + 1) : setActiveSection(idx + 2)}
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xs font-bold text-violet-600 bg-violet-50 px-2.5 py-1 rounded-full">
            Section {SYNTHETIC_SECTIONS.length + 1} of {SYNTHETIC_SECTIONS.length + 1}
          </span>
          <h4 className="font-bold text-navy text-base" style={{ fontFamily: "Georgia, serif" }}>
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100">
          <button
            type="button"
            onClick={() => setActiveSection(SYNTHETIC_SECTIONS.length)}
            className="text-sm font-semibold text-gray-400 hover:text-navy transition-colors">
            ← Back
          </button>
          <div />
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 mb-6">
          ...
          <div className="flex items-center gap-3">
            {hasDraft && (
              <span className="text-xs text-gray-400 font-medium">
                {allLocked
                  ? "All sections locked ✓"
                  : `${lockedCount}/${SYNTHETIC_SECTIONS.length} sections locked`}
              </span>
            )}
        ...
        {!hasDraft && !generating && (
          <p className="text-sm text-gray-400 py-4 text-center">
            Click &ldquo;Generate AI Draft&rdquo; to build the personas and simulate responses using Claude.
          </p>
        )}

        {hasDraft && !generating && (
          <div>
            {activeSection >= 1 && activeSection <= SYNTHETIC_SECTIONS.length
          ...
          <div className="pt-5 mt-4 border-t border-dashed border-seagreen/30 flex items-center gap-3 flex-wrap">
            ...
            {!allLocked && (
              <p className="text-xs text-amber-600 font-medium">
                Lock all {SYNTHETIC_SECTIONS.length} sections to enable download and send
                ({lockedCount}/{SYNTHETIC_SECTIONS.length} locked)
              </p>
            )}
