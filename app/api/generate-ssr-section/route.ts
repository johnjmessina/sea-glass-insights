...
import { generateSSRSection } from "@/lib/claudeServices";

export const maxDuration = 60;

export async function POST(req: NextRequest) {
  if (!process.env.ANTHROPIC_API_KEY) {
    console.error("[generate-ssr-section] ANTHROPIC_API_KEY is not set");
  ...
  try {
    ...
    console.error(`[generate-ssr-section] "${sectionKey}" failed for order`, orderId, "—", err);
  ...
  if (saveError) {
    console.error("[generate-ssr-section] Failed to save section to Supabase:", saveError);
  }
