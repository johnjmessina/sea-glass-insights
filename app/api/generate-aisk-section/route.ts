...
import { generateAISKSection } from "@/lib/claudeServices";

export const maxDuration = 60;

export async function POST(req: NextRequest) {
  if (!process.env.ANTHROPIC_API_KEY) {
    console.error("[generate-aisk-section] ANTHROPIC_API_KEY is not set");
  ...
  const timeoutPromise = new Promise<never>((_, reject) =>
    setTimeout(
      () => reject(new Error("Section timed out. Use the retry button.")),
      55_000
    )
  );
  ...
  try {
    ...
    console.error(`[generate-aisk-section] "${sectionKey}" failed for order`, orderId, "—", err);
  ...
  if (saveError) {
    console.error("[generate-aisk-section] Failed to save section to Supabase:", saveError);
  }
