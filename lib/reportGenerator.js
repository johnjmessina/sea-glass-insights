...
// logo_cover.png    — primary horizontal lockup (ratio 2.06:1)
// logo_icon.png     — icon only for header (ratio 1.502:1)
function loadLogos() {
  const logoDir = path.join(process.cwd(), 'public', 'logos');
  return {
    cover: fs.readFileSync(path.join(logoDir, 'logo_cover.png')),
    icon:  fs.readFileSync(path.join(logoDir, 'logo_icon.png')),
  };
}
  ...
  const doc = new Document({
    ...
    // ══ COVER PAGE ════════════════════════════════════════
    {
      ...
      children: [
        ...
        // Logo — centered, ratio 2.06:1
        new Paragraph({
          alignment: AlignmentType.CENTER,
          spacing: { before: 0, after: 360 },
          children: [new ImageRun({
            data: logos.cover,
            transformation: { width: 400, height: 194 },
            type: 'png'
          })]
        }),
                ...
                children: [
                  ...
                  new TableCell({
                    ...
                    children: [new Paragraph({
                      ...
                      children: [new ImageRun({
                        data: logos.icon,
                        transformation: { width: 72, height: 48 },
                        type: 'png'
                      })]
                    })]
                  }),
