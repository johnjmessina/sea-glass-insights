 ...
 */
async function generateReport(orderData, aiContent, analystNote, executiveSummary = '') {
  ...
  const doc = new Document({
    ...
    // ══ REPORT BODY ═══════════════════════════════════════
    {
      ...
      children: [
        ...
        // Only rendered when executiveSummary is non-empty.
                  ...
                  children: [
                    new Paragraph({ spacing: { before: 0, after: 80 },
                      children: [new TextRun({ text: "Executive Summary",
                        bold: true, size: 28, font: CG, color: NAVY })] }),
                    new Paragraph({ spacing: { before: 0, after: 0 },
                      children: [new TextRun({ text: executiveSummary.trim(),
                        size: 22, font: MT, color: INK })] }),
                  ]
                })
              ]
            })]
